...
tipoListaPaginas* criaListaSimulacao(tipoLista **l){
	...
	while(pAtual != NULL){
		...
		simulaEstatisticasFuturo(&processoFuturo);
		simulaEstadoFuturo(&processoFuturo);
...
void gerenciaPaginas(tipoQuadro *q, unsigned int idRecebida, unsigned int pagRecebida, unsigned int tempoRecebido ,tipoLista **l){
	...
	for(i = 0;i < TAMQUADROS;i++){
		...
		if(q->p[i].id == 0 && q->p[i].nPagina == 0 && q->p[i].tempo == 0 && q->temQuadroLivre == 0){
			q->temQuadroLivre = 1;
			nQuadroLivre = i;
		}
	}
    ...
    if(pAtual != NULL){
		...
		while(pgAtual != NULL){
			if(pgAtual->pagina.tempo == pAtual->processo.tempoExecutando || pgAtual->pagina.nPagina == 0){
				gerenciaPaginas(q, pgAtual->pagina.id, pgAtual->pagina.nPagina, pgAtual->pagina.tempo, l);
			}
			pgAtual = pgAtual->proximo;
		}
